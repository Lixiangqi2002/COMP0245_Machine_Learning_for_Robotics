...
def plot_objective(
    ...
    cmap='viridis_r',
    ax=None,
    save_contour_plots=False,
    save_diagonal_plots=False,  # New argument to control saving
    ...
    for i in range(n_dims):
        for j in range(n_dims):
            ...
            if i == j:
                ...
                # Save the plot if save_diagonal_plots is True
                if save_diagonal_plots:
                    plt.figure()
                    plt.plot(xi, yi)
                    plt.axvline(minimum[index], linestyle="--", color="r", lw=1)
                    plt.text(minimum[index], max(yi), f'{minimum[index]:.2f}', color='red',
                             verticalalignment='bottom', horizontalalignment='right')
                    plt.title(f'Partial dependence of dimension {i}')
                    plt.savefig(f"{save_dir}/diagonal_plot_{i}_objective.png")
                    plt.close()
                ...
                if save_contour_plots:
                    if index1 == 0 or index2 == 0:
                        plt.figure()
                        plt.contourf(xi, yi, zi, levels, cmap=cmap)
                        result.x_iters = np.array(result.x_iters)
                        plt.scatter(result.x_iters[:, index2], result.x_iters[:, index1], c='k', s=10)
                        plt.scatter(x_vals[index2], x_vals[index1], c=['r'], s=100, lw=0.0, marker='*')
                        # plt.title(f'Contour plot for dimension 1 (vs dimension {j + 1})')
                        # plt.savefig(f"{save_dir}/contour_kp0_dimension_1_vs_{j + 1}.png")
                        # plt.close()
                        if index1 == 0:
                            plt.title(f'Contour plot: kp0 (dimension 1) vs dimension {j + 1}')
                            plt.savefig(f"{save_dir}/contour_kp0/contour_kp0_vs_dimension_{j + 1}.png")
                        else:
                            plt.title(f'Contour plot: dimension {i + 1} vs kp0 (dimension 1)')
                            plt.savefig(f"{save_dir}/contour_kp0/contour_dimension_{i + 1}_vs_kp0.png")
                        plt.close()
                    ...
                    if index1 == 7 or index2 == 7:
                        ...
                        plt.scatter(result.x_iters[:, index2], result.x_iters[:, index1], c='k', s=10)
                        plt.scatter(x_vals[index2], x_vals[index1], c=['r'], s=100, lw=0.0, marker='*')
                        # plt.title(f'Contour plot for dimension 8 (vs dimension {j + 1})')
                        # plt.savefig(f"{save_dir}/contour_kd0_dimension_8_vs_{j + 1}.png")
                        # plt.close()
                        if index1 == 7:
                            plt.title(f'Contour plot: kd0 (dimension 1) vs dimension {j + 1}')
                            plt.savefig(f"{save_dir}/contour_kd0/contour_kd0_vs_dimension_{j + 1}.png")
                        else:
                            plt.title(f'Contour plot: dimension {i + 1} vs kd0 (dimension 1)')
                            plt.savefig(f"{save_dir}/contour_kd0/contour_dimension_{i + 1}_vs_kd0.png")
                        plt.close()
    ...
    plt.savefig(f"{save_dir}/plot_total_objectives.png")
...
def plot_evaluations(
    ...
    for i in range(n_dims):
        for j in range(n_dims):
            if i == j:
                ...
                max_idx = np.argmax(n)
                max_x = (bins_[max_idx] + bins_[max_idx + 1]) / 2
                max_y = n[max_idx]


                ax_.text(max_x, max_y, f'({max_x:.2f}, {max_y:.0f})',
                ...
                if save_diagonal_plots:
                    ...
                    plt.text(max_x, max_y, f'({max_x:.2f}, {max_y:.0f})',
                             verticalalignment='bottom', horizontalalignment='center', color='black')
                    plt.title(f'Histogram of dimension {i}')
                    plt.savefig(f"{save_dir}/diagonal_plot_{i}_evaluations.png")
                    plt.close()
                ...
                if save_scatter_plots:
                    if index_i == 0 or index_j == 0:
                        plt.figure()
                        plt.scatter(samples[:, index_j], samples[:, index_i], c=order, s=40, lw=0.0, cmap=cmap)
                        plt.scatter(minimum[index_j], minimum[index_i], c=['r'], s=100, lw=0.0, marker='*')
                        # plt.title(f'Scatter plot for dimension 1 (vs dimension {j + 1})')
                        # plt.savefig(f"{save_dir}/scatter_kp0_dimension_1__vs_{j + 1}.png")

                        if index_i == 0:
                            plt.title(f'Scatter plot: kp0 (dimension 1) vs dimension {j + 1}')
                            plt.savefig(f"{save_dir}/scatter_kp0/scatter_kp0_vs_dimension_{j + 1}.png")
                        else:
                            plt.title(f'Scatter plot: dimension {i + 1} vs kp0 (dimension 1)')
                            plt.savefig(f"{save_dir}/scatter_kp0/scatter_dimension_{i + 1}_vs_kp0.png")
                        plt.close()


                    if index_i == 7 or index_j == 7:
                        plt.figure()
                        plt.scatter(samples[:, index_j], samples[:, index_i], c=order, s=40, lw=0.0, cmap=cmap)
                        plt.scatter(minimum[index_j], minimum[index_i], c=['r'], s=100, lw=0.0, marker='*')
                        # plt.title(f'Scatter plot for dimension 8 (vs dimension {j + 1})')
                        # plt.savefig(f"{save_dir}/scatter_kd0_dimension_8_vs_{j + 1}.png")
                        # plt.close()
                        if index_i == 7:
                            plt.title(f'Scatter plot: kd0 (dimension 1) vs dimension {j + 1}')
                            plt.savefig(f"{save_dir}/scatter_kd0/scatter_kd0_vs_dimension_{j + 1}.png")
                        else:
                            plt.title(f'Scatter plot: dimension {i + 1} vs kd0 (dimension 1)')
                            plt.savefig(f"{save_dir}/scatter_kd0/scatter_dimension_{i + 1}_vs_kd0.png")
                        plt.close()
    ...
    plt.savefig(f"{save_dir}/plot_total_evaluations.png")
    return ax
